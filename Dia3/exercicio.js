...
const getPopulation = () => countries
  .reduce((acc, cur) => cur.population + acc, 0);
...
const getTotalArea = () => countries
  .reduce((acc, cur) => cur.area + acc, 0);

console.log(getTotalArea());

//3

const longetName = () => countries
  .reduce((acc, cur) => (acc.name.length < cur.name.length) ? cur : acc);

console.log(longetName());
...
const countA = () => names.join('').toLowerCase().split('')
...
const studentAverage = () => students
  .map((element, index) => {
    return {name: element, average: grades[index]
